...
namespace WindowsFormsApp1
{
    public partial class Breed : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    string Query = "insert into Breeding  values('"+ date.ToString("MM-dd-yyyy") + "', "+DogIdbox.SelectedValue.ToString()+",N'"+DogNameTxt.Text+"' , '"+ date2.ToString("MM-dd-yyyy")+"','"+ date3.ToString("MM-dd-yyyy") + "' , '"+DogAgeTxt.Text+"',N'"+RemarksTxt.Text+"')";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    DateTime date2 = Convert.ToDateTime(PregDateTxt.Value);
                    DateTime date3 = Convert.ToDateTime(DateBornedTxt.Value);
                    string Query = "update Breeding  set BreedDate= '" + date.ToString("MM-dd-yyyy") + "' ,DogId="+DogIdbox.SelectedValue.ToString()+",DogName= N'"+DogNameTxt.Text+"',PregDate= '"+date2.ToString("MM-dd-yyyy")+"',DateBorned= '"+date3.ToString("MM-dd-yyyy")+"',DogAge= '"+DogAgeTxt.Text+"',Remarks= N'"+RemarksTxt.Text+"'  where BreedId =" + key + ";";
                    SqlCommand cmd = new SqlCommand(Query, conn);
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Thanh cong");
                    conn.Close();
        ...
        {
            conn.Open();
            string Query = "SELECT TOP (1000) [BreedId] N'id',[BreedDate] N'Ngày lai',[DogId] N'Mã số chó',[DogName] N'Tên chó',[PregDate] 'Ngày mang thai',[DateBorned] 'Ngày đẻ',[DogAge] 'Tuổi chó',[Remarks]'Đánh giá'FROM[QLGC].[dbo].[Breeding]";
